...
wget https://raw.githubusercontent.com/Pakatube/Nginx-conf/refs/heads/main/check-ip.sh -O /root/aws/check-ip.sh
cat > /root/aws/changeip.sh << EOF
#!/bin/bash
...
exit 0
EOF
screen -dmS ipcheck bash -c 'watch -n 1 bash /root/aws/check-ip.sh'
